...
// Unlocked is what Unparse renders a nil (TRUE_BOOLEXP) expression
// as, and what GETLOCKSTR shows for an object with no lock set.
// PROP_UNLOCKED_VAL upstream.
const Unlocked = "*UNLOCKED*"

// Unparse renders a lock expression back to text, the format Parse's
// dbload path accepts. This is unparse_boolexp/unparse_boolexp1.
//
// With fullname false, dbrefs render as "#123" — this is the form
// stored in a Lock property, and the one Parse's dbload path expects
// back. With fullname true, dbrefs render the way host.Name shows
// them to viewer, for display to a player (e.g. PRETTYLOCK). viewer
// is unused when fullname is false, since that form never consults
// it.
