...
// COPYPLAYER, TOADPLAYER, PNAME_HISTORY, COPYOBJ, PROGRAM_SETLINES and DUMP.
//
// Every one of these is mlev 4 with upstream's own generic "Requires Wizbit."
// wording, so mlev_gen.go's generated floor gates them before these run and
// none needs an inline check — except TOADPLAYER, whose refusal to run from
// inside a @force is its own and has no generated equivalent.
func init() {
	...
	register("TOADPLAYER", func(f *Frame) (*Result, error) {
		...
		// Unlike every other wizard primitive here, this one refuses to run
		// from inside a @force at all: a wizard tricked into forcing a
		// program should not be able to delete a player by doing so.
		if h.ForceLevel() > 0 {
		...
		if v, ok := h.GetProp(victim, noRecycleProp); ok && !v.IsEmpty() {
	...
	register("PNAME_HISTORY", func(f *Frame) (*Result, error) {
		...
		d := NewDict()
		// The history is recorded whatever this parameter says; the
		// parameter only decides whether a program may read it back.
	...
	register("COPYOBJ", func(f *Frame) (*Result, error) {
		...
		if f.MLevel() < 3 && h.Owner(src) != h.Owner(f.Prog.Ref) {
			return nil, errf("Permission denied.")
		}
		f.alreadyCreated++
		// Only a wizard's copy carries the source's hidden properties over.
	...
	register("PROGRAM_SETLINES", func(f *Frame) (*Result, error) {
		...
		prog := progV.Ref
		if !h.Valid(prog) || h.ObjType(prog) != ref.TypeProgram {
		...
		for _, v := range linesV.Array.Values() {
			...
			// An empty line is stored as a single space, so a saved
			// program never has a line that reads as end-of-text.
			if v.Str == "" {
				lines = append(lines, " ")
				continue
			}
	...
	register("DUMP", func(f *Frame) (*Result, error) {
		...
		h.DumpNow()
		// Upstream reports 0 when a dump is already running and this one was
		// skipped. Emerald's flush is a background write that never blocks
		// the world, so there is nothing to collide with and no reason to
...
// pnameHistoryDir is upstream's PNAME_HISTORY_PROPDIR, and noRecycleProp its
// NO_RECYCLE_PROP.
const (
