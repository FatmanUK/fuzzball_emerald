...
func TestStrEncryptDecryptRoundTrips(t *testing.T) {
	// Upstream's own scramble table only maps letters and a
	// couple of control characters onto a fixed 97-character
	// "visible ASCII" range — its own doc comment says as much
	// — so only printable text, what STRENCRYPT is actually
	// used for, is expected to round-trip.
...
// pronounTestHost stubs just enough of Host for pronounSub: TuneGet
// (the gender_prop name) and GetProp (the gender value itself).
type pronounTestHost struct {
	Host
	gender string
}

func (h *pronounTestHost) TuneGet(string) (string, bool) {
	return "sex", true
}
func (h *pronounTestHost) Name(ref.Ref) string {
	return "Igor"
}
...
func TestPronounSubUsesDefaultTableByGender(t *testing.T) {
	// No gender set -> the object is referred to by name
	// throughout, with "'s" for the possessive forms.
	...
	// A capitalised directive capitalises the substitution's
	// first letter — upstream's own "isupper(prn[1])" rule —
	// a lowercase one leaves it be.
	...
	// %% is a literal percent. An unrecognised directive loses
	// its '%' and keeps only the letter, which is what upstream's
	// own default case writes — not the two characters as
	// typed.
