...
func TestCanCallRejectsNonProgramArg(t *testing.T) {
	...
	if err == nil ||
		err.Error() != "Invalid program dbref argument. (1)" {
...
func TestCanCallRejectsEmptyName(t *testing.T) {
	...
	_, err := prims[PrimNumber("CANCALL?")](f)
	if err == nil ||
		err.Error() != "Invalid string argument. Must be non-null. (2)" {
...
func TestCanCallForwardsToHostWithProgUID(t *testing.T) {
	...
	if call.callerLevel != 3 || call.callerUID != 20 ||
		call.prog != testProgram2 || call.name != "foo" {
...
// TestKillOwnPIDAbortsSilently checks upstream's do_abort_silent
// special case: killing your own pid ends the program via
// errSilentAbort rather than an ordinary error, and never reaches
// Host.KillPID.
...
// TestForkGatedByGeneratedMlevFloor checks FORK's mlev gate at the
// layer it actually runs at: mlev_gen.go's "FORK": 3 (a genuine
// unconditional floor, unlike KILL's own ownership-escaping check),
// enforced by the dispatcher in primitive() before FORK's own
// primFunc ever runs — which is why FORK's primFunc has no mlev
// check of its own to test directly, unlike KILL's.
...
// TestForkBuildsAnIndependentChildStartingAfterItself checks the
// pieces FORK's own primFunc is responsible for, on top of fork()'s
// own tested deep-copy: the child's PC lands one past the FORK
// instruction, it starts with a 0 already on its stack, and it is the
// exact frame handed to Host.Fork — not some other copy.
func TestForkBuildsAnIndependentChildStartingAfterItself(t *testing.T) {
	...
	// The primitive dispatcher advances f.PC itself after a plain
	// primFunc returns; FORK's own primFunc must not have touched
	// it, so it is still sitting on the FORK instruction here.
	if f.PC != 10 {
	...
	// The parent's own stack (still holding its 123) must be
	// untouched; the child got its own copy plus the pushed 0.
	if len(f.Stack) != 1 || f.Stack[0].Num != 123 {
		t.Errorf("parent stack = %+v, should be unaffected by the fork", f.Stack)
	}
	if len(child.Stack) != 2 || child.Stack[0].Num != 123 ||
		child.Stack[1].Num != 0 {
...
func TestQueueRejectsNonProgramArg(t *testing.T) {
	...
	_, err := prims[PrimNumber("QUEUE")](f)
	if err == nil ||
		err.Error() != "Invalid program dbref argument (2)." {
		t.Fatalf("err = %v, want the invalid-program message", err)
	}
	if len(h.queueCalls) != 0 {
...
func TestQueueForwardsToHostAndPushesResult(t *testing.T) {
	...
	call := h.queueCalls[0]
	if call.descr != 9 || call.prog != testProgram2 ||
		call.seconds != 5 || call.arg != "hello" {
		t.Fatalf("unexpected call: %+v", call)
	}
}

// TestQueueTreatsANonStringArgAsEmpty mirrors upstream's own
// type-blind read of oper1->data.string: any non-string value's Str
// field is Go's zero value "" anyway, so QUEUE need not (and does
// not) check the type explicitly.
...
func TestForceRejectsEmptyCommand(t *testing.T) {
	...
	_, err := prims[PrimNumber("FORCE")](f)
	if err == nil ||
		err.Error() != "Empty command argument (2)." {
...
func TestForceRejectsCarriageReturn(t *testing.T) {
	...
	_, err := prims[PrimNumber("FORCE")](f)
	if err == nil ||
		err.Error() != "Carriage returns not allowed in command string. (2)." {
...
func TestForceForwardsToHostWithNoStackEffect(t *testing.T) {
	...
	call := h.forceCalls[0]
	if call.descr != 5 || call.player != 20 ||
		call.program != testProgram ||
		call.victim != testThing || call.command != "look" {
		t.Fatalf("unexpected call: %+v", call)
	}
}
...
func TestForcedByArrayPushesHostResult(t *testing.T) {
	...
	vals := v.Array.Values()
	if len(vals) != 2 || vals[0].Ref != testProgram ||
		vals[1].Ref != testPlayer {
		t.Fatalf("array = %+v, want [%d %d]", vals, testProgram, testPlayer)
	}
}

// TestForceFamilyRejectsBelowMlevelFourWithWizbitWording pins the
// wording discovered via golden: FORCE, FORCEDBY and FORCEDBY_ARRAY
// all abort with upstream's own "Wizbit only primitive.", not the
// generic dispatcher message most other level-4 primitives get (see
// gen_mlev.py's CUSTOM_ABORT_MESSAGE) — which is also why these
// three check their own mlev inline instead of relying on
// primMLevel/mlev_gen.go.
func TestForceFamilyRejectsBelowMlevelFourWithWizbitWording(t *testing.T) {
	for _, name := range []string{"FORCE", "FORCEDBY", "FORCEDBY_ARRAY"} {
		t.Run(name, func(t *testing.T) {
			...
			if err == nil ||
				err.Error() != "Wizbit only primitive." {
...
func TestGetPIDsRejectsBelowMlevelThreeWithItsOwnWording(t *testing.T) {
	...
	if err == nil ||
		err.Error() != "Permission denied.  Requires Mucker Level 3." {
...
func TestGetPIDsForwardsArgAndPushesResult(t *testing.T) {
	...
	vals := v.Array.Values()
	if len(vals) != 3 || vals[0].Num != 3 || vals[1].Num != 7 ||
		vals[2].Num != 12 {
		t.Fatalf("array = %+v, want [3 7 12]", vals)
	}

	if len(h.getPIDsCalls) != 1 ||
		h.getPIDsCalls[0].obj != testProgram2 {
		t.Fatalf("GetPIDs calls = %+v, want obj = #%d", h.getPIDsCalls, testProgram2)
	}
}

// TestGetPIDsAppendsOwnPIDOnlyForItsOwnProgram checks GETPIDS's own
// final step, upstream's "if (program == ref) push fr->pid": the
// calling frame's pid is appended only when the argument is exactly
// the calling program's own ref, not for any other match —
// including upstream's "ref < 0" wildcard, which golden caught this
// not applying to.
func TestGetPIDsAppendsOwnPIDOnlyForItsOwnProgram(t *testing.T) {
	...
	if len(h.getPIDsCalls) != 1 ||
		h.getPIDsCalls[0].selfPID != 42 {
		t.Fatalf("GetPIDs should have been asked to exclude pid 42: %+v", h.getPIDsCalls)
	}

	// The wildcard match does not trigger the append — only an
	// exact match on the calling program's own ref does.
	h2 := newLockTestHost()
	f2 := newTestFrame(h2)
	f2.PID = 7
...
// TestGetPIDsAcceptsAnyDbrefIncludingNonexistent checks that, unlike
// most other primitives taking an object argument, GETPIDS does not
// validate the dbref exists — upstream's own prim_getpids checks
// only that the value is dbref-typed at all, per its "no permission
// checking is done" doc comment on get_pids.
...
// TestGetPIDInfoRejectsNonIntegerArg checks GETPIDINFO's own
// argument-type check, upstream's "Non-integer argument (1)" — no
// trailing period, unlike most of this file's other argument-type
// messages, matching the C exactly.
...
// TestGetPIDInfoRejectsBelowMlevelThreeForOtherPid checks
// GETPIDINFO's own conditional mlev check, upstream's "mlev < 3 &&
// oper1->data.number != fr->pid" — a program below mucker level 3
// may not inspect a pid other than its own.
...
// TestGetPIDInfoAllowsBelowMlevelThreeForOwnPid checks the escape
// hatch: a program may always inspect its own pid regardless of
// mucker level, which is exactly what kept GETPIDINFO out of
// mlev_gen.go's generated table.
...
// TestGetPIDInfoSelfBranchBuildsFromLiveFrame checks the self
// branch's deliberately reproduced upstream quirks: CALLED_DATA is
// always "", NEXTRUN is always 0, SUBTYPE is always "", and MLEVEL
// reports the real effective level — exactly prim_getpidinfo's own
// hardcoding, not a gap.
func TestGetPIDInfoSelfBranchBuildsFromLiveFrame(t *testing.T) {
	...
	if filters, ok := v.Array.Get(Str("FILTERS")); !ok ||
		filters.Type != TypeArray ||
		len(filters.Array.Values()) != 0 {
...
// TestGetPIDInfoOtherBranchForwardsToHostAndBuildsDict checks the
// other-pid branch: it reads Host.PIDInfo and fills
// PID/CPU/FILTERS/TYPE/MLEVEL itself — MLEVEL hardcoded to 0,
// matching upstream's own get_pidinfo, which is its own documented
// TODO, not an Emerald gap.
...
// TestGetPIDInfoReturnsEmptyDictWhenPIDNotFound matches upstream
// leaving a freshly allocated new_array_dictionary untouched when
// neither the timequeue nor the MUF-event queue know the pid.
...
// TestWatchPIDForwardsToHostWhenTargetExists checks that a live
// target is handled entirely by Host.WatchPID, with no event queued
// on the calling frame — that only happens in the "target does not
// exist" branch.
func TestWatchPIDForwardsToHostWhenTargetExists(t *testing.T) {
	...
	if len(h.watchPIDCalls) != 1 ||
		h.watchPIDCalls[0] != (watchPIDCall{1, 2}) {
...
// TestWatchPIDQueuesProcExitEventWhenTargetDoesNotExist checks
// upstream's else branch: watching a pid that names no live process
// queues a PROC.EXIT.<pid> event directly on the caller's own frame,
// immediately, rather than ever registering a wait.
func TestWatchPIDQueuesProcExitEventWhenTargetDoesNotExist(t *testing.T) {
	...
	if ev.Name != "PROC.EXIT.999" ||
		ev.Data.Type != TypeInteger || ev.Data.Num != 999 {
		t.Fatalf("event = %+v, want PROC.EXIT.999 carrying 999", ev)
	}
}

// TestEventWaitForServesAnAlreadyQueuedEvent checks that
// EVENT_WAITFOR does not unconditionally block: an event queued
// before it runs — exactly what WATCHPID does when the target is
// already gone — is served immediately.
