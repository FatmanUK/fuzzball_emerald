...
//
// Every mlev floor in src/p_connects.c has its own wording rather than the
// generic dispatcher's "Permission denied."/"Permission denied.  Requires
// Wizbit." — three distinct level-3 variants and two level-4 variants
// appear across this one file, discovered while porting Phase 3 — so every
// primitive here checks its own floor inline rather than leaning on
// mlev_gen.go and primMLevel the way an unconditional floor with the
// dispatcher's own generic wording would (see gen_mlev.py's
// CUSTOM_ABORT_MESSAGE). That includes ONLINE, ONLINE_ARRAY, DESCRDBREF and
// DESCRSECURE?, ported before this phase with the generic wording; fixed
// here to match the C alongside everything new.
func init() {
	...
	// players: one player may be connected several times. Unlike most of
	// this file, prim_descrcount has no mlev floor at all.
	...
	// DESCR is the connection the program was started from. Unlike most of
	// this file, prim_descr has no mlev floor at all.
	...
	// DESCRIDLE and DESCRTIME are ports of prim_descr_idle/prim_descr_time,
	// upstream's pdescridle/pdescrontime.
	...
	// prim_descr_least_idle/prim_descr_most_idle. Unlike DESCRIDLE/
	// DESCRTIME, neither one aborts on a -1 result — upstream's own
	// primitives push it straight through, "no connection found" simply
	// being a valid answer for "which of a player's connections...".
	register("DESCRLEASTIDLE", descrPlayerStat(Host.DescrLeastIdle))
	register("DESCRMOSTIDLE", descrPlayerStat(Host.DescrMostIdle))

	// DESCRHOST and DESCRUSER are ports of prim_descr_host/prim_descr_user.
	...
	// DESCRNOTIFY is a port of prim_descr_notify. The misspelled "an
	// string" in its argument-2 message is upstream's own literal wording,
	...
	// NEXTDESCR is a port of prim_nextdescr. Unlike most of this file it
	// never aborts on an invalid descriptor number — 0 already means
	// "nothing next", the same answer an invalid one gets.
	...
	// FIRSTDESCR and LASTDESCR are ports of prim_firstdescr/prim_lastdescr —
	// see Host.FirstDescr/Host.LastDescr's own doc comment for the
	...
	register("DESCR_SETUSER", func(f *Frame) (*Result, error) {
		...
		if whoV.Type != TypeObject || (whoV.Ref != ref.Nothing && !isValidPlayer(h, whoV.Ref)) {
		...
		if whoV.Ref != ref.Nothing && !h.CheckPassword(whoV.Ref, passV.Str) {
			return nil, errf("Incorrect password.")
		}
		return nil, f.Push(Bool(h.SetUser(int(descrV.Num), whoV.Ref)))
	})

	// DESCRFLUSH is a port of prim_descrflush. Upstream computes a result
	// from pdescrflush but never pushes it — genuinely stack-neutral,
	// consuming its descriptor argument and returning nothing at all.
...
// isValidPlayer is upstream's valid_player: a live dbref of type player.
func isValidPlayer(h Host, r ref.Ref) bool {
	return h.Valid(r) && h.ObjType(r) == ref.TypePlayer
}

// popDescr pops an integer descriptor number, upstream's own "Argument not
// an integer. <arg>" wording parameterised by which argument this was.
...
// descriptor is pushed first, so it sits under the size on the stack).
...
// descrIntStat builds DESCRIDLE, DESCRTIME and DESCRBUFSIZE: a descriptor
// number in, an int out, aborting when get reports it invalid with a
// negative result — upstream's own convention for all three.
...
// descrStringStat builds DESCRHOST and DESCRUSER: a descriptor number in, a
// string out, aborting when the descriptor does not exist.
...
// descrPlayerStat builds DESCRLEASTIDLE and DESCRMOSTIDLE: a player dbref
// in, an int out, with no invalid-result abort — see their own registration
// comment.
...
func descrByPlayer(get func(Host, ref.Ref) int) primFunc {
	return func(f *Frame) (*Result, error) {
		...
		if v.Type != TypeObject || (v.Ref != ref.Nothing && !isValidPlayer(h, v.Ref)) {
