...
func init() {
	...
	register("SMATCH", func(f *Frame) (*Result, error) {
		...
		return nil, f.Push(Bool(smatch(s, pattern)))
	})
	...
	register("TIMEFMT", func(f *Frame) (*Result, error) {
		...
		return nil, f.Push(Str(strftime(format, time.Unix(secs, 0).UTC())))
	})
...
// smatch implements MUF's pattern matching, which is not a regular expression:
// '*' matches any run, '?' any single character, '{a|b}' any alternative, and
// everything is case-insensitive.
func smatch(s, pattern string) bool {
	return smatchAt(ascii.Fold(s), ascii.Fold(pattern))
}
