...
// Telnet option negotiation, enough of it to read lines reliably and
// to learn the client's window size.
//
// Fuzzball also negotiates STARTTLS (option 46). Emerald does not:
// every listener is already TLS, so there is no cleartext session to
// upgrade.
...
// telnetDecoder strips telnet control sequences from a byte stream,
// leaving the application data, and reports window sizes as it learns
// them.
type telnetDecoder struct {
	...
	subOpt byte
	subBuf []byte
	// pendingDup tracks the doubled 255 that represents a literal
	// 255 inside a subnegotiation.
	pendingDup bool
...
// Decode consumes raw bytes and returns the application data among
// them.
	...
	for _, c := range in {
		...
		case stateIAC:
			...
			case SB:
				...
				d.subBuf = d.subBuf[:0]
			default:
				// Every other command is a no-op for
				// us.
				d.state = stateNormal
			}
		...
		case stateSubnegIAC:
			switch c {
			case IAC:
				// A doubled 255 inside a
				// subnegotiation is a literal 255.
				d.appendSub(IAC)
				d.state = stateSubneg
			case SE:
				d.endSubneg()
...
// appendSub adds a byte to the subnegotiation buffer, bounded so a
// client cannot make us allocate without limit.
...
func (d *telnetDecoder) endSubneg() {
	if d.subOpt == OptNAWS && len(d.subBuf) >= 4 &&
		d.onResize != nil {
...
// initialNegotiation is what the server offers on connect: ask the
// client to report its window size.
func initialNegotiation() []byte {
	return []byte{IAC, DO, OptNAWS}
}

// escapeIAC doubles any 255 in outgoing text, which the protocol
// requires so it is not read as a command.
...
// NewDecoder returns a decoder that reports window-size changes to
// onResize.
...
// EncodeLine renders one line for the wire: escaped, and terminated
// the way the line protocol expects.
