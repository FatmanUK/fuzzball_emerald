// Package ref defines the database reference type and the object type
// and flag bits that Fuzzball stores alongside every object.
//
// Values here are wire-compatible with Fuzzball 7: the flag bit
// positions and the special reference numbers are read verbatim from
// legacy database dumps, so they must not be renumbered.
package ref
...
// Special references. These are negative so they can never collide
// with a real object index.
const (
...
// Ok reports whether r is a plausible index into the object table. It
// does not prove the object exists; callers holding a world still
// need to look it up.
