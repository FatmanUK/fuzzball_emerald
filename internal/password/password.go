...
//
// Emerald writes Argon2id. It also verifies the two formats Fuzzball 7 wrote,
// so an imported world's players can still log in, and upgrades them in place
...
// Argon2id parameters. These follow the OWASP recommendation of 19 MiB and
// two passes, which is comfortable for a login that happens once per session.
const (
	argonTime    = 2
...
const (
	...
	// Upstream derives 57 bytes into a 128-byte buffer and then writes only
	// the first 56 as hex. Deriving 56 gives the same bytes, because both
	// fit inside PBKDF2's first output block.
	legacyPBKDF2Len = 56
)
...
const (
	...
	legacyPBKDF2 = "$1$"
	// LegacyMD5Prefix tags a bare base64 MD5 digest carried over from a
	// dump. The tag is ours: upstream stored the digest with no marker at
	// all, which is indistinguishable from a corrupt field.
	LegacyMD5Prefix = "$fbmd5$"
)

// NoPassword is the stored form for a player with no usable credential.
//
// Fuzzball treats an empty password as "any password works", which is how a
// player with no password logs in there. Emerald refuses the login instead:
// silently accepting any password for an account is not a behaviour worth
// preserving. Such players need a password set before they can connect.
const NoPassword = ""
...
type Result struct {
	...
	OK bool
	// NeedsUpgrade reports that the password was correct but was stored in
	// a legacy format, so the caller should rehash and save it.
	NeedsUpgrade bool
}

// Verify checks plain against a stored hash.
//
// Unlike upstream, which compares only strlen(computed) leading bytes and so
// accepts a stored hash that merely starts with the right digest, this
// compares the whole value in constant time.
...
// verifyLegacyMD5 checks a bare base64 MD5 digest from an old Fuzzball world.
...
// FromLegacyDump converts the password field of a dump record into a stored
// hash. An empty field becomes NoPassword.
