...
// DESCRHOST and DESCRUSER's own successful-path values are
// environment- dependent — a container's view of the peer address,
// and (per DESCRUSER's own doc comment in internal/muf/frame.go) an
// OS-assigned ephemeral port upstream reports and Emerald has no
// equivalent for — so this checks shape (a live descriptor answers
// some string) and the argument-validation wording, both of which are
// deterministic, rather than content.
...
// TestConnectsMatchesFuzzball checks DESCRHOST and DESCRUSER against
// the C server — the two primitives in Phase 3 that need mlevel 4,
// so cannot run in the shared "connects" case in golden_test.go,
// which compiles at 3.
