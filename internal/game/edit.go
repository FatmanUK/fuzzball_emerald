...
// The editor's commands, from include/edit.h. Each is a single
// letter, and only the first character of the last word on the line
// is looked at, which is why "3 5 d" and "3 5 delete" both delete
// lines 3 to 5.
const (
	insertCommand     = 'i'
	deleteCommand     = 'd'
	...
	// exitInsert leaves insert mode and returns to the command
	// level.
	exitInsert = "."
	// maxEditArg is how many arguments an editor command takes,
	// which upstream fixes at MAX_ARG.
	maxEditArg = 2
)

// editSession is one player's editing state.
//
// The program's text lives here, not in the object, so an editor
// session costs nothing until someone opens one and an abandoned
// session cannot corrupt the stored source. The program is marked
// INTERNAL for as long as a session is open, which is what stops two
// people editing it at once.
type editSession struct {
	program ref.Ref
	lines   []string
	// curr is the insertion point, upstream's curr_line: a
	// one-based line number that new text is placed after.
	curr int
	// insert is set between "i" and ".".
	insert bool
}

// edit puts a player into the editor for a program. This is
// edit_program.
func (s *Server) edit(c *ctx, program ref.Ref) {
	...
	e := &editSession{
		...
		// Upstream keeps the current line on the program
		// rather than the session, so reopening an editor
		// resumes where the last one left off. It is not
...
// splitSource breaks stored source into editable lines. A program
// saved with a trailing newline must not gain an empty last line from
// it.
...
func (s *Server) editing(who ref.Ref) *editSession {
	return s.editors[who]
}

// editInput handles one line typed by a player who is in the editor.
// It reports whether the line was consumed, which it always is: the
// editor sees every line until the player leaves it.
//
// The line arrives untrimmed, because in insert mode leading
// whitespace is part of the program text and an empty line is a blank
// line to insert.
...
//
// The parse is upstream's: the line is split into at most three
// words, each read as a number, and the command is the first
// character of the last non-empty word. "def" is special-cased before
// that, because its definition is the rest of the line rather than a
// word.
func (s *Server) editCommand(c *ctx, e *editSession, line string) {
	...
	// "def" is only a macro definition once a second word follows
	// it: upstream recognises it while parsing that word, so a
	// bare "def" never reaches the special case and is read as
...
// parseEditLine splits an editor command into its words and their
// numeric values. It reports false when the line is rejected
// outright.
func parseEditLine(c *ctx, line string) ([]string, []int, bool) {
	...
	for i, w := range words {
		// Upstream uses atoi, which reads a leading number
		// and ignores the rest, so "3d" is line 3 and the
		// delete command at once.
		n := leadingInt(w)
		if n < 0 {
...
// leadingInt reads the integer at the front of a word, as atoi does:
// no digits means zero, and trailing text is ignored.
...
// parseDef recognises a macro definition, "def <name> <body>". The
// body is the rest of the line verbatim, so a definition may contain
// spaces.
...
func (s *Server) defineMacro(c *ctx, name, body string) {
	// A macro whose name starts with a digit or a dot could never
	// be typed, because '.' introduces the expansion and a number
	// is a literal.
...
// listMacros prints the macro table, or the part of it between two
// names.
//
// The bounds are compared only as far as the bound is long, which is
// upstream's strncmp: "aaa" as both ends selects every macro whose
// name starts with "aaa", not the single macro called "aaa". One
// bound is used as both ends, and none means the whole table.
//
// The full form gives each macro's definition and owner; the short
// form packs names into fixed columns and breaks the line once it
// passes 70 characters, which is why a row holds five names rather
// than a round number.
func (s *Server) listMacros(c *ctx, bounds []string, full bool) {
	...
	for _, m := range c.w.Macros() {
		if boundedBefore(m.Name, first) ||
			boundedAfter(m.Name, last) {
			continue
		}
		if full {
...
// boundedBefore reports whether a name sorts before the lower bound,
// comparing only as many bytes as the bound has.
...
// compareN compares name against the whole of bound, ignoring
// anything in name beyond the bound's length. This is strncmp(name,
// bound, strlen(bound)).
...
//
// Where the line lands is upstream's, and its edge cases matter: a
// current line past the end of the program appends, a current line of
// zero inserts before the first line, and an empty line is stored as
// a single space so the listing shows something.
func (s *Server) insertLine(c *ctx, e *editSession, line string) {
	...
	case e.curr < 0 || e.curr-1 >= len(e.lines):
		// Walking off the end appends, and so does a negative
		// current line: upstream's walk never terminates on
		// one, so it runs out of list instead.
...
// editRange resolves the argument forms every line-taking command
// shares: no argument means the current line, one means that line
// alone, two are a range.
...
// listProgram prints lines from the editing buffer. A nil arg lists
// everything, which is what entering the editor does.
//
// When sysmsg is set the counts and complaints are wrapped in MUF
// comment parentheses, so the output of "@list !" can be pasted
// straight back into a program.
...
// closeEditor drops the session and clears the flags that mark it
// open.
...
func (s *Server) compileEdited(c *ctx, e *editSession) {
	...
	// The compile is cached under the program's ref, so a caller
	// that runs it before the editor saves gets the version just
	// checked. Cancelling the session drops it again.
	s.cacheProgram(e.program, prog, nil)
	c.tell("Program compiled successfully.")
}

// compileForEditor compiles text and reports the failure to the
// player.
//
// A compile error is always shown to someone in the editor, whichever
// command triggered it: upstream's do_abort_compile prints rather
...
// do_abort_compile does, because that wording is what programmers
// read.
...
// viewHeader shows another program's leading comment block, which is
// the convention for documenting what a library does and how to call
// it.
...
// viewable reports whether a player may read a program's source:
// either they control it, or it is set VEHICLE, which for a program
// means "viewable".
...
// toggleNumbers turns the listing's line numbers on or off. Upstream
// keeps this on the player's INTERNAL flag, so it survives between
// sessions.
...
func (s *Server) listPublics(c *ctx, e *editSession, arg []int, argc int) {
	...
	// The buffer is what the programmer is asking about when it
	// is their own session; anything else is compiled from what
	// is stored.
...
//
// It never compiles anything: upstream reads the code the program
// already carries, so "u" before a successful "c" says there is
// nothing there rather than reporting a compile error.
//
...
// debugging aid rather than something a program reads, so that
// difference is left as it is.
...
// describeInst renders one instruction the way upstream's disassemble
// does.
...
// scopedVarName names a function-scoped variable slot, which needs
// the procedure the instruction belongs to.
...
// isWizard reports whether a player holds wizard powers right now,
// which a quelled wizard does not.
...
// cmdProgram implements @program: find or create a program, then edit
// it.
func (s *Server) cmdProgram(c *ctx) {
	...
	if !s.requireNotGuest(c, "@program") ||
		!s.requireMucker(c, "@program") {
		return
	}
...
func (s *Server) cmdEdit(c *ctx) {
	...
	if !s.requireNotGuest(c, "@edit") ||
		!s.requireMucker(c, "@edit") {
		return
	}
...
// matchProgram resolves a program name the way @program and @edit do:
// what the player is carrying, what is in the room, a registered
// name, or a dbref.
...
// cmdList implements @list, which prints a program's stored source
// without entering the editor.
//
// The line specification may be prefixed with '!' to comment out the
// counts, '#' to force line numbers on, or '@' to force them off.
// Those prefixes are how a programmer pastes a listing back into a
// program.
func (s *Server) cmdList(c *ctx) {
	...
	// The numbering override applies for this listing only, so it
	// is set and put back rather than saved.
	if numbers != nil {
...
// listOptions strips the '!', '#' and '@' prefixes from a line
// specification.
...
// parseLineSpec reads "12", "12-20", "12-" or "-20" into a range. An
// empty specification lists the whole program.
...
// requireMucker reports whether the player may write programs,
// telling them if not. This is upstream's MUCKERONLY: any mucker bit
// at all will do.
...
// requireNotGuest refuses a command to a guest account, as NOGUEST
// does.
...
// applyTuneFlags sets flags named by a tune string: one letter per
// flag, with a digit giving the mucker level. This is
// set_flags_from_tunestr.
func applyTuneFlags(o *world.Object, spec string) {
	for i := 0; i < len(spec); i++ {
		...
		// '0' is accepted and ignored, as upstream has it:
		// clearing the level this way would fight the cap
		// applied afterwards.
		if ch == '0' {
			continue
		}
...
// tuneFlagLetters maps the letters new_program_flags accepts to their
// bits. 'M' is a mucker level rather than a flag and is handled with
// the digits, and 'W' is ignored: upstream will not auto-set the
// wizard bit.
...
// editorHelpText is the command summary "h" prints. Upstream reads
// this from a help file on disk; a single binary has nowhere to read
// one from, so it is built in.
