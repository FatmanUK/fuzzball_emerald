...
// cmdSanity checks the object graph for inconsistency and reports what it
// finds, changing nothing.
...
// requireNotForced refuses a command that arrived through @force. A command
// that can rewrite ownership must be typed by the person taking
// responsibility for it, not reached through something they were tricked into
// forcing.
...
//
// This is the tool of last resort, for damage @sanfix cannot work out how to
// repair. It does no validation at all, which is the point: it can put the
// database into a state nothing else can, and can just as easily make things
// worse.
func (s *Server) cmdSanchange(c *ctx) {
	if !s.requireGod(c, "@sanchange") || !s.requireNotForced(c, "@sanchange") {
		return
	}
	...
	// Upstream reads both numbers with sscanf and does not check that they
	// parsed, so anything unreadable comes through as whatever was in the
	// variable — which for the target means zero, and is then rejected by
	...
	if field == nil {
		// Only players and things have a home. Upstream writes this
		// complaint to the server's own output rather than to the
		// player, so the player sees nothing at all; that silence is
		// reproduced, because a transcript is compared against it.
...
// sanField points at the reference a field name names, along with the label
// the change is reported under. A nil field with ok set means the object has
// no such reference to change.
//
// The label for "exits" says "next field", which is wrong and is upstream's:
// the two cases were written by copying one from the other. It is reproduced
// because the message is what a transcript is compared against.
...
// requireGod gates the commands that can damage the database outright. These
// are God-only upstream rather than wizard-only, because a wizard who could
// run them could rewrite ownership and so make themselves God.
